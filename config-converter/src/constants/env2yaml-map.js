...
const ENV_TO_YAML_MAPPING = {
  ...
  APIML_ALLOW_ENCODED_SLASHES: function(val, envs, yamlConfig) {
    return _.set(yamlConfig, 'components.gateway.apiml.service.allowEncodedSlashes', stringToBoolean(val));
  },
  APIML_CORS_ENABLED: function(val, envs, yamlConfig) {
    return _.set(yamlConfig, 'components.gateway.apiml.service.corsEnabled', stringToBoolean(val));
  },
  ...
  APIML_SECURITY_X509_ENABLED: function(val, envs, yamlConfig) {
   return _.set(yamlConfig, 'components.gateway.apiml.security.x509.enabled', stringToBoolean(val));
  },
  ...
  EXTERNAL_COMPONENTS: function(val, envs, yamlConfig) {
    return enableComponents(val, yamlConfig);
  },
  ...
  LAUNCH_COMPONENT_GROUPS: function(val, envs, yamlConfig) {
    ...
    return enableComponents(components, yamlConfig);
  },
  ...
  ZOWE_APIM_NONSTRICT_VERIFY_CERTIFICATES: function(val, envs, yamlConfig) {
    _.set(yamlConfig, 'components.gateway.apiml.security.ssl.nonStrictVerifySslCertificatesOfServices', stringToBoolean(val));
    _.set(yamlConfig, 'components.discovery.apiml.security.ssl.nonStrictVerifySslCertificatesOfServices', stringToBoolean(val));
    _.set(yamlConfig, 'components.gateway.apiml.security.ssl.nonStrictVerifySslCertificatesOfServices', stringToBoolean(val));
    _.set(yamlConfig, 'components.caching-service.apiml.security.ssl.nonStrictVerifySslCertificatesOfServices', stringToBoolean(val));
    return yamlConfig;
  },
  ...
  ZOWE_EXPLORER_HOST: function(val, envs, yamlConfig) {
    return appendToArray(val, "zowe.externalDomains", yamlConfig);
  },
  ...
  ZOWE_ZSS_SERVER_TLS: function(val, envs, yamlConfig) {
   return _.set(yamlConfig, 'components.zss.tls', stringToBoolean(val));
  },
  ...
  CACHING_STORAGE_REDIS_SENTINEL_NODES: function(val, envs, yamlConfig) {
   return _.set(yamlConfig, 'components.caching-service.storage.redis.sentinel.nodes', String(val).split(/,/));
  },
  CACHING_STORAGE_REDIS_SSL_ENABLED: function(val, envs, yamlConfig) {
   return _.set(yamlConfig, 'components.caching-service.storage.redis.ssl.enabled', stringToBoolean(val));
  },
  ...
  ZWE_REFERRER_HOSTS: function(val, envs, yamlConfig) {
    return appendToArray(val, "zowe.referrerHosts", yamlConfig);
  },
